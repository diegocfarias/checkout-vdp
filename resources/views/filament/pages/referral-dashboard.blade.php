<x-filament-panels::page>
    <div style="display:flex; flex-direction:column; gap:24px;">
        ...
        @if(count($ranking) > 0)
            <x-filament::section heading="Ranking de Afiliados">
                <table style="width:100%; font-size:14px; border-collapse:collapse;">
                    <thead>
                        <tr style="border-bottom:1px solid rgba(128,128,128,0.2);">
                            <th class="fi-section-header-description" style="padding:10px 16px; text-align:left;">#</th>
                            <th class="fi-section-header-description" style="padding:10px 16px; text-align:left;">Afiliado</th>
                            <th class="fi-section-header-description" style="padding:10px 16px; text-align:center;">Código</th>
                            <th class="fi-section-header-description" style="padding:10px 16px; text-align:center;">Indicações</th>
                            <th class="fi-section-header-description" style="padding:10px 16px; text-align:right;">GMV</th>
                            <th class="fi-section-header-description" style="padding:10px 16px; text-align:right;">Créditos</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($ranking as $i => $row)
                            <tr style="border-top:1px solid rgba(128,128,128,0.1);">
                                ...
                                <td style="padding:10px 16px; text-align:center;">
                                    <span style="display:inline-block; padding:2px 10px; background:rgba(37,99,235,0.15); color:#60a5fa; font-size:12px; font-weight:600; border-radius:9999px;">{{ $row['referral_code'] }}</span>
                                </td>
                                <td style="padding:10px 16px; text-align:center; font-weight:500;">{{ $row['count'] }}</td>
                                <td style="padding:10px 16px; text-align:right; color:inherit;">{{ $row['gmv'] }}</td>
                                <td style="padding:10px 16px; text-align:right; font-weight:500; color:inherit;">{{ $row['credits'] }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
