...
<div class="combination-card bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-all duration-200 animate-fadeIn"
    ...
    <div class="flex flex-col lg:flex-row">
        ...
        <div class="flex-1 min-w-0">
            ...
            <div class="lg:hidden sticky top-14 z-10 flex items-center justify-between px-5 py-3 bg-white border-b border-gray-100 rounded-t-xl">
                <div class="whitespace-nowrap">
                    @if(($pixEnabled ?? false) && ($pixDiscount ?? 0) > 0)
                        ...
                        <div class="flex items-center gap-1.5">
                            ...
                            <span class="text-[10px] font-semibold bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded-full">PIX</span>
                        </div>
                    @else
                        <p class="text-lg font-bold text-gray-900">R$ {{ number_format($totalPrice, 2, ',', '.') }}</p>
                    @endif
                    <p class="text-[11px] text-gray-400">{{ $hasInbound ? 'ida + volta' : 'por adulto' }}</p>
                </div>
        ...
        {{-- Coluna direita: preço e botão (desktop) --}}
        <div class="hidden lg:flex lg:w-52 shrink-0 border-l border-gray-100">
            <div class="sticky top-20 self-start w-full p-5 flex flex-col items-center justify-center text-center">
                @if(($pixEnabled ?? false) && ($pixDiscount ?? 0) > 0)
                    @php $pixPrice = $pixPrice ?? round($totalPrice * (1 - ($pixDiscount / 100)), 2); @endphp
                    <p class="text-sm text-gray-400 line-through">R$ {{ number_format($totalPrice, 2, ',', '.') }}</p>
                    <p class="text-2xl font-bold text-emerald-600 whitespace-nowrap">R$ {{ number_format($pixPrice, 2, ',', '.') }}</p>
                    <span class="text-xs font-semibold bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full mt-1">PIX</span>
                @else
                    <p class="text-2xl font-bold text-gray-900 whitespace-nowrap">R$ {{ number_format($totalPrice, 2, ',', '.') }}</p>
                @endif
                <p class="text-xs text-gray-400 mt-1">{{ $hasInbound ? 'ida + volta' : 'por adulto' }}</p>

                <form action="{{ route('search.select') }}" method="POST" class="group-form w-full mt-4" data-group="{{ $groupIdx }}">
                    @csrf
                    <input type="hidden" name="search_id" value="{{ $searchId }}">
                    <input type="hidden" name="outbound" class="selected-ob" value="{{ json_encode($obFlights[0] ?? [], JSON_UNESCAPED_UNICODE) }}">
                    @if($hasInbound)
